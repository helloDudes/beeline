...
class EventHandlerBitrix24 extends CBitrixComponent
{
    /**
     ...
     *
     */
    function AddEvent($arEvent, $arATEData) {

        $user = $this->GetUserData($arATEData["ate_key"], $arEvent["targetId"]);
        ...
        $arOptions["CRM_SOURCE"] = ExternalApi::MULTICHANNEL_STATUS_ID_PREFIX.$arEvent["multicallNumber"];
        ...
        if(!$arCall["result"]["CALL_ID"]) {
            return;
        }

        //Добавляем параметры события в БД
        $entity_data_class = DbInteraction::GetEntityDataClass($this->arParams["BITRIX24_EVENTS_HL_BLOCK_ID"]);
     ...
     * @param array $arATEData параметры подписки
     */
    function CallCreate($arEvent, $arATEData) {
        $user = $this->GetUserData($arATEData["ate_key"], $arEvent["targetId"]);
        if(!$user)
            return;
        ...
        if($arEvent["acdUserId"] || $call["UF_IS_HUNT_GROUP"]) {
            ...
            if($arEvent["answerTimeUNIX"]) {
                $this->CallCenterCallRegister($arEvent, $arATEData, $arAdd);
            }
            else {
        ...
        $record_url = ExternalApi::GetRecordURL(
            ...
            ExternalApi::RECORD_WAITING_TIME
        );
     ...
     */
    function GetUserData($ate_key, $targetID) {
     ...
     *
     * @param object $redirections массив переадресаций
     ...
     */
    function CheckRedirects($redirections, $arEvent, &$multicallNumber, &$isHuntGroup = false) {
     ...
     */
    function EventScan($element, &$arOptions, &$arFeatures, &$arRecord) {
     ...
     */
    function CallCenterCallRegister($arEvent, $arATEData, $arAdd) {
        ...
        //Проверяем сначала последний элемент, а потом все остальные из БД
        if($arAdd) {
            $this->EventScan($arAdd, $arOptions, $arFeatures, $arRecord);
        }
        while($el = $rsData->Fetch()) {
            $this->EventScan($el, $arOptions, $arFeatures, $arRecord);
        }
        ...
        if($arFeatures["is_answered"]) {
            ...
            $arOptions["STATUS_CODE"]="603-S";
            $manager = $this->GetManager($arATEData["ate_key"]);
            if($manager)
                $arOptions["USER_ID"]=$manager["UF_CRM_USER_ID"];
        }

        if(!$arOptions["USER_ID"])
        {
            return;
        }
        ...
        //Добавляем запрос на создание звонка
        $arQueries = array();
        $arQueries["externalcall_finish"]="telephony.externalcall.finish?".http_build_query($arOptions);

        //Если нет короткого номера менеджера, берём из БД
        ...
        //Добавляем запрос на создание задачи
        ...
        //Отсылаем все созданные запросы
        ExternalApi::RestCommand(
            $arATEData,
            "batch",
            array(
     ...
     */
    function AddRedirect($clientNumber, $extension, $arATEData, $crmUserId, $leadId) {
        $add = "INSERT INTO ".$this->arParams["BITRIX24_LEADS_HL_BLOCK_NAME"];
    ...
    public function executeComponent()
    {
        ...
        if(
            ...
            || !$this->arParams["BITRIX24_EVENTS_HL_BLOCK_ID"]
            || !$this->arParams["BITRIX24_LEADS_HL_BLOCK_NAME"]
        ...
        $this->arParams["BITRIX24_EVENTS_HL_BLOCK_ID"] = intval($this->arParams["BITRIX24_EVENTS_HL_BLOCK_ID"]);
        $this->arParams["BITRIX24_LEADS_HL_BLOCK_NAME"] = strval($this->arParams["BITRIX24_LEADS_HL_BLOCK_NAME"]);
        $this->arParams["REDIRECT_URI"] = strval($this->arParams["REDIRECT_URI"]);

        //Начинаем парсить параметры xml в массив $arEvent
        $xml = str_replace('<?xml version="1.0" encoding="UTF-8"?>', "", file_get_contents('php://input'));

        $xmlData = new SimpleXMLElement($xml);
        $xmlDataChilds = $xmlData->children("xsi", TRUE);
        $eventsList = $xmlDataChilds->eventData->attributes("xsi1", true);
        $event = trim(strval($eventsList[0]));
        ...
        if (($arEvent["abonentNumber"] && $arEvent["clientNumber"])) {
            if ($arEvent["event"] == "CallReceivedEvent" || $arEvent["event"] == "CallOriginatedEvent")
                $this->AddEvent($arEvent, $arATEData);
            if ($arEvent["event"] == "CallReleasedEvent")
                $this->CallCreate($arEvent, $arATEData);
        };
        ...
        if (
            ...
            if ($arEvent["releasingParty"] == "remoteRelease")
                $this->CallCenterCallRegister($arEvent, $arATEData);
        }
        //Если сломалась действующая подписка, чиним
        if ($arEvent["event"] == "SubscriptionTerminatedEvent") {
            ExternalApi::RepairSubscribe(array(
                "token" => $arATEData["beeline_token"],
                "life_span" => ExternalApi::SUBSCRIBE_LIFE_SPAN,
                "handler_url" => ExternalApi::BEELINE_SERVER_NAME . "/calls_analytic/include/beeline_connection/beeline_event_handler_bitrix24.php",
                "id" => $arATEData["ate_id"],
